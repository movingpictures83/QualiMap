...
void QualiMapPlugin::output(std::string file) {
 ...
 // qualimap bamqc -bam assembly_map_long-reads.bam -outfile Qualimap_map_long.pdf
	std::string command = "qualimap bamqc -bam "+inputfile+" -outfile "+file.substr(file.find_last_of('/')+1)+"; cp "+std::string(PluginManager::prefix())+"/*stats/*.pdf "+std::string(PluginManager::prefix());
